...
# SolisenseFile <- "../Data/ImportedData/ImportedSolisenseData/BalticCCA_Imported_SolisenseLight_LRC_CaloxyAll.Rds"  
# 
...
```{r read ProcessFile Sol}
SolisenseFile <- "../Data/ImportedData/ImportedSolisenseData/BalticCCA_Imported_SolisenseLight_LRC_Caloxy.Rds"  
...
#colnames(SolFitsMetaCaloxy)
```


```{r read ProcessFile Sol}
SolisenseFile <- "../Data/ImportedData/ImportedSolisenseData/BalticCCA_Imported_SolisenseLight_LRC_CaloxyBiolSteps.Rds"  
...
# Merge Solisense data from doubletap, Caloxy, and CaloxyBiolSteps

```{r}

SolFitsMetaAll <-rbind(SolFitsMeta, SolFitsMetaCaloxy, SolFitsMetaBiolSteps, SolFitsMetaCaloxyAll)

#To keep the df name
SolFitsMeta <-SolFitsMetaAll
```
...
SolFitsMeta<-SolFitsMetaPhase  
  
```

#Removind weird outliers
```{r}
...
SolFitsMeta<-rbind(SolFitsMeta1, SolFitsMeta2)
```
...
SolFitsMeta %>%
  ...
  ggplot() +
  geom_point(aes(x = ActPARCorr, y = Sigmax, colour = as.factor(Phase)), size = 3, show.legend = T) +
  geom_line(aes(x = ActPARCorr, y = Sigmax, linetype = as.factor(Phase)), size = 0.3, show.legend = T) +
  geom_vline(xintercept = 30, linetype = "dotted", size = 0.7) +
  scale_fill_identity() +
...
SolFitsMeta %>%
  mutate(WLNum = if_else(WL == "WW", 750, as.numeric(WL))) %>%
  ggplot() +
  geom_point(aes(x = ActPARCorr, y = Sigdark, colour = as.factor(Phase)), size = 3, show.legend = T) +
  geom_line(aes(x = ActPARCorr, y = Sigdark, linetype = as.factor(Phase)), size = 0.3, show.legend = T) +
  geom_vline(xintercept = 30, linetype = "dotted", size = 0.7) +
  scale_fill_identity() +
  ggh4x::facet_nested(cols = vars(Ex_WL), rows = vars(Strain), labeller = label_parsed) +
  theme_bw() 
...
SolFitsMeta %>%
  mutate(WLNum = if_else(WL == "WW", 750, as.numeric(WL))) %>%
  ggplot() +
  geom_point(aes(x = ActPARCorr, y = JVPSII_umol_eLs, shape = as.factor(Phase)), size = 3, show.legend = T) +
  geom_line(aes(x = ActPARCorr, y = JVPSII_umol_eLs, linetype = as.factor(Phase)), size = 0.3, show.legend = T) +
  geom_vline(xintercept = 30, linetype = "dotted", size = 0.7) +
  scale_fill_identity() +
  ggh4x::facet_nested(cols = vars(Ex_WL), rows = vars(Strain), labeller = label_parsed) +
  theme_bw() 
...
SolFitsMeta %>%
  ...
  ggplot() +
  geom_point(aes(x = ActPARCorr, y = Alp1QA, colour = as.factor(Phase)), size = 3, show.legend = T) +
  geom_line(aes(x = ActPARCorr, y = Alp1QA, linetype = as.factor(Phase)), size = 0.3, show.legend = T) +
  #geom_vline(xintercept = 90, linetype = "dotted", size = 0.7) +
  scale_fill_identity() +
  ggh4x::facet_nested(cols = vars(Par_ue, WL), rows = vars(Strain, Ex_WL), labeller = label_parsed) +
  theme_bw() 
...
SolFitsMeta %>%
  ...
  filter(Ex_WL!= 470 & Ex_WL != 505) %>% 
  ggplot() +
  geom_point(aes(x = ActPARCorr, y = Alp2QA, colour = as.factor(Phase)), size = 3, show.legend = T) +
  geom_line(aes(x = ActPARCorr, y = Alp2QA, linetype = as.factor(Phase)), size = 0.3, show.legend = T) +
  #geom_vline(xintercept = 90, linetype = "dotted", size = 0.7) +
  scale_fill_identity() +
  ggh4x::facet_nested(cols = vars(Par_ue, WL), rows = vars(Strain, Ex_WL), labeller = label_parsed) +
  theme_bw() 
...
# Group to get single LRC

```{r}
colnames(SolFitsMeta)

SolFitsMetaFilter <- SolFitsMeta %>%
  #filter(Ex_WL == 445) %>% 
  dplyr::group_by(SampleID, Run, Par_ue, WL, Ex_WL, E_days, ObsDate, FilenameSolisense) %>% 
  #filter(Dark1s==1) %>% 
  #filter(Tau1QA == min(Tau1QA, na.rm = TRUE)) %>% 
  #filter(ActPAR == 20) %>% 
  dplyr::ungroup() 
...
SolFitsMetaFilter %>%
  mutate(WLNum = if_else(WL == "WW", 750, as.numeric(WL))) %>%
    filter(Ex_WL!= 470 & Ex_WL != 505) %>% 
  ggplot() +
  geom_point(aes(x = E_days, y = Tau1QAdark, colour = as.factor(Phase)), size = 3, show.legend = T) +
  ggh4x::facet_nested(cols = vars(Par_ue, WL), rows = vars(Strain, Ex_WL), labeller = label_parsed) +
  theme_bw() 
...
SolFitsMetaFilter %>%
  mutate(WLNum = if_else(WL == "WW", 750, as.numeric(WL))) %>%
    filter(Ex_WL!= 470 & Ex_WL != 505) %>% 
  ggplot() +
  geom_point(aes(x = E_days, y = Tau1QAmin, colour = as.factor(Phase)), size = 3, show.legend = T) +
  ggh4x::facet_nested(cols = vars(Par_ue, WL), rows = vars(Strain, Ex_WL), labeller = label_parsed) +
  theme_bw() 
...
SolFitsMetaFilter %>%
  mutate(WLNum = if_else(WL == "WW", 750, as.numeric(WL))) %>%
    filter(Ex_WL!= 470 & Ex_WL != 505) %>% 
  ggplot() +
  geom_point(aes(x = E_days, y = TauAvdark, colour = as.factor(Phase)), size = 3, show.legend = T) +
  ggh4x::facet_nested(cols = vars(Par_ue, WL), rows = vars(Strain, Ex_WL), labeller = label_parsed) +
  theme_bw() 
...
SolFitsMetaFilter %>%
  mutate(WLNum = if_else(WL == "WW", 750, as.numeric(WL))) %>%
    filter(Ex_WL!= 470 & Ex_WL != 505) %>% 
  ggplot() +
  geom_point(aes(x = E_days, y = TauAvmin, colour = as.factor(Phase)), size = 3, show.legend = T) +
  geom_line(aes(x = E_days, y = TauAvmin), size = 1, show.legend = F) +
  ggh4x::facet_nested(cols = vars(Par_ue, WL), rows = vars(Strain, Ex_WL), labeller = label_parsed) +
  theme_bw() 
...
SolFitsMetaFilter %>%
  mutate(WLNum = if_else(WL == "WW", 750, as.numeric(WL))) %>%
    filter(Ex_WL!= 470 & Ex_WL != 505) %>% 
  ggplot() +
  geom_point(aes(x = E_days, y = Alp3QAdark, colour = as.factor(Phase)), size = 3, show.legend = T) +
  ggh4x::facet_nested(cols = vars(Par_ue, WL), rows = vars(Strain, Ex_WL), labeller = label_parsed) +
  theme_bw() 
...
SolFitsMetaFilter %>%
  mutate(WLNum = if_else(WL == "WW", 750, as.numeric(WL))) %>%
    filter(Ex_WL!= 470 & Ex_WL != 505) %>% 
  ggplot() +
  geom_point(aes(x = E_days, y = Alp3QAmin, colour = as.factor(Phase)), size = 3, show.legend = T) +
    geom_line(aes(x = E_days, y = Alp3QAmin), size = 1, show.legend = F) +
  ggh4x::facet_nested(cols = vars(Par_ue, WL), rows = vars(Strain, Ex_WL), labeller = label_parsed) +
  theme_bw() 
...
SolFitsMetaFilter %>%
  ...
  filter(Ex_WL!= 470 & Ex_WL != 505) %>% 
  ggplot() +
  geom_point(aes(x = E_days, y = Sigdark, colour = as.factor(Phase)), size = 3, show.legend = T) +
  ggh4x::facet_nested(cols = vars(Par_ue, WL), rows = vars(Strain, Ex_WL), labeller = label_parsed) +
  theme_bw() 
...
SolFitsMetaFilter %>%
  ...
  filter(Ex_WL!= 470 & Ex_WL != 505) %>% 
  ggplot() +
  geom_point(aes(x = E_days, y = Sigmax, colour = as.factor(Phase)), size = 3, show.legend = T) +
  ggh4x::facet_nested(cols = vars(Par_ue, WL), rows = vars(Strain, Ex_WL), labeller = label_parsed) +
  theme_bw() 
...
```{r}
SolFitsMetaLRC<-SolFitsMetaFilter %>%
  filter(Sig>0) %>% 
  filter(Sigdark>0) %>% 
  filter(Sigmax>0)
```
...
mutate(Sigmax_nm2psii=Sigmax_m2psii*1000000000000000000)  
```
...
SolFitsMetaLRC %>%
  ...
  filter(Phase != "St") %>% 
  ggplot() +
  geom_point(aes(x = ActPARCorr, y = Sigdark, colour = as.factor(Phase)), size = 2, show.legend = T) +
  ggh4x::facet_nested(cols = vars(WL, Par_ue), rows = vars(Strain, Ex_WL), labeller = label_parsed) +
  theme_bw()
...
SolFitsMetaLRC %>%
  ...
  filter(Phase != "St") %>% 
  ggplot() +
  geom_point(aes(x = ActPARCorr, y = Sigmax, colour = as.factor(Phase)), size = 2, show.legend = T) +
  ggh4x::facet_nested(cols = vars(WL, Par_ue), rows = vars(Strain, Ex_WL), labeller = label_parsed) +
  theme_bw()
...
SolFitsMetaLRC %>%
  ...
  filter(Phase != "St") %>% 
  ggplot() +
  geom_point(aes(x = ActPARCorr, y = TauAvdark, colour = as.factor(Phase)), size = 2, show.legend = T) +
  ggh4x::facet_nested(cols = vars(WL, Par_ue), rows = vars(Strain, Ex_WL), labeller = label_parsed) +
  theme_bw()
...
SolFitsMetaLRC %>%
  ...
  filter(Phase != "St") %>% 
  ggplot() +
  geom_point(aes(x = ActPARCorr, y = TauAvmin, colour = as.factor(Phase)), size = 2, show.legend = T) +
  ggh4x::facet_nested(cols = vars(WL, Par_ue), rows = vars(Strain, Ex_WL), labeller = label_parsed) +
  theme_bw()
...
SolFitsMetaLRC %>%
  ...
  filter(Phase != "St") %>% 
  ggplot() +
  geom_point(aes(x = E_days, y = qpOxbo, colour = as.factor(Phase)), size = 2, show.legend = T) +
  ggh4x::facet_nested(cols = vars(WL, Par_ue), rows = vars(Strain, Ex_WL), labeller = label_parsed) +
  theme_bw()
...
SolFitsMetaLRC %>%
  ...
  #filter(Phase != "St") %>% 
  ggplot() +
  geom_point(aes(x = E_days, y = ETRqpOxbo, colour = as.factor(Phase)), size = 2, show.legend = T) +
  ggh4x::facet_nested(cols = vars(WL, Par_ue), rows = vars(Strain, Ex_WL), labeller = label_parsed) +
  theme_bw()
...
# Check if it is not a error here!! check if filename is needed here!
...
SolFitsMetaLRC <- SolFitsMetaLRC %>%
  ...
  group_by(Strain, Ex_WL, Par_ue, Photoperiod, WLNum, WL, O2, E_days, ObsDate, ActPARCorr, Phase) %>%
  
  summarize(SampleID, Ex_WL, FilenameSolisense, Run, Strain, ExpDate, Par_ue, Photoperiod, O2, WL, WLNum,  PARPhotonDose_day, ObsDate, ObsTime, ObsDateTime, Sig, Sig_nm2psii, ActPARCorr, ActPARCorr_photonsm2s, Sigdark_nm2psii, Sigmax_nm2psii, FoOxbo, qpOxbo, JVPSII_umol_eLs, E_days, TauAv, TauAvmin, TauAvdark, Tau1QAmin, Tau1QAdark, Tau3QAmin, Tau3QAdark, qpOxbo, ETRqpOxbo, Phase,
            ...
            meanSigmax_nm2psii = mean(Sigmax_nm2psii),
            sdSigmax_nm2psii = sd(Sigmax_nm2psii),
            
  
            meanTauAv = mean(TauAv),
            sdTauAv = sd(TauAv),
            
            meanTauAvmin = mean(TauAvmin),
            sdTauAvmin = sd(TauAvmin),
            
            meanTauAvdark = mean(TauAvdark),
            sdTauAvdark = sd(TauAvdark),
            
            meanTau1QAmin = mean(Tau1QAmin),
            sdTau1QAmin = sd(Tau1QAmin),
            
            meanTau1QAdark = mean(Tau1QAdark),
            sdTau1QAdark = sd(Tau1QAdark),
            
            meanTau3QAmin = mean(Tau3QAmin),
            sdTau3QAmin = sd(Tau3QAmin),
            
            meanTau3QAdark = mean(Tau3QAdark),
            sdTau3QAdark = sd(Tau3QAdark),
...
# Calculate Sigma Allmean measured on all days from different phase of growth 
...
SolFitsMetaLRC <- SolFitsMetaLRC %>%
  #filter(E_days>=4) %>% 
  #filter(E_days <=10) %>% 
  group_by(Strain, Ex_WL, Par_ue, Photoperiod, WLNum, WL, O2, ActPARCorr, Phase) %>%
  
  summarize(SampleID, Ex_WL, FilenameSolisense, Run, Strain, ExpDate, Par_ue, Photoperiod, O2, WL, WLNum,  PARPhotonDose_day, ObsDate, ObsTime, ObsDateTime, Sig, Sig_nm2psii, meanSig_nm2psii, sdSig_nm2psii, meanSigdark_nm2psii, sdSigdark_nm2psii, meanSigmax_nm2psii, sdSigmax_nm2psii, ActPARCorr, ActPARCorr_photonsm2s, Sigdark_nm2psii, FoOxbo, qpOxbo, JVPSII_umol_eLs, E_days, TauAv, qpOxbo, meanTauAv, sdTauAv, meanTauAvmin, sdTauAvmin, meanTauAvdark, sdTauAvdark, 
            meanTau1QAmin, sdTau1QAmin, meanTau1QAdark, sdTau1QAdark, 
            meanTau3QAmin, sdTau3QAmin, meanTau3QAdark, sdTau3QAdark, 
            
            meanqpOxbo, sdqpOxbo, meanJVPSII_umol_eLs, sdJVPSII_umol_eLs, ETRqpOxbo, meanETRqpOxbo, sdETRqpOxbo, Phase,
            ...
            AllmeanSigmax_nm2psii = mean(meanSigmax_nm2psii),
            AllsdSigmax_nm2psii = sd(sdSigmax_nm2psii),
            
            AllmeanTauAv = mean(meanTauAv),
            AllsdTauAv = sd(sdTauAv),
            
            AllmeanTauAvmin = mean(meanTauAvmin),
            AllsdTauAvmin = sd(sdTauAvmin),
            
            AllmeanTauAvdark = mean(meanTauAvdark),
            AllsdTauAvdark = sd(sdTauAvdark),
            
            AllmeanTau1QAmin = mean(meanTau1QAmin),
            AllsdTau1QAmin = sd(sdTau1QAmin),
            
            AllmeanTau1QAdark = mean(meanTau1QAdark),
            AllsdTau1QAdark = sd(sdTau1QAdark),
            
            AllmeanTau3QAmin = mean(meanTau3QAmin),
            AllsdTau3QAmin = sd(sdTau3QAmin),
            
            AllmeanTau3QAdark = mean(meanTau3QAdark),
            AllsdTau3QAdark = sd(sdTau3QAdark),
...
SolFitsMetaLRC %>%
  ...
  #filter(Strain == "PC-rich") %>% 
  #filter(O2 == 0) %>% 
  filter(WL == 450 | WL == 530 |  WL == 660) %>%
  filter(Ex_WL == 445 | Ex_WL == 535 | Ex_WL == 590) %>%
  ggplot() +
  geom_point(aes(x = ActPARCorr, y = AllmeanSig_nm2psii, colour = as.factor(Phase)), size = 2, show.legend = T) +
...
SolFitsMetaLRC %>%
  ...
  filter(Ex_WL == 445 | Ex_WL == 535 | Ex_WL == 590) %>%
  ggplot() +
  geom_point(aes(x = ActPARCorr, y = meanSigmax_nm2psii, colour = as.factor(Phase)), size = 2, show.legend = T) +
  # geom_point(aes(x = ActPARCorr, y = meanSig_nm2psii), colour = "darkgreen",size = 2, show.legend = T) +
  # geom_point(aes(x = ActPARCorr, y = AllmeanSig_nm2psii), colour = "red",size = 2, show.legend = T) +
  scale_fill_identity() +
  ggh4x::facet_nested(cols = vars(WL, Par_ue), rows = vars(Strain, Ex_WL), labeller = label_parsed) +
...
SolFitsMetaLRC %>%
  ...
  ggplot() +
  geom_point(aes(x = ActPARCorr, y = AllmeanSig_nm2psii, colour = as.factor(Phase)), size = 2, show.legend = T) +
  geom_line(aes(x = ActPARCorr, y = AllmeanSig_nm2psii, linetype = as.factor(Phase)), size = 0.3, show.legend = F) +
  #geom_point(aes(x = ActPARCorr, y = meanSig_nm2psii), colour = "darkgreen",size = 2, show.legend = T) +
  #geom_point(aes(x = ActPARCorr, y = AllmeanSig_nm2psii), colour = "red",size = 2, show.legend = T) +
...
SolFitsMetaLRC %>%
  ...
  filter(Ex_WL == 445 | Ex_WL == 535 | Ex_WL == 590) %>%
    filter(Phase!= "St") %>% 
  ggplot() +
  geom_point(aes(x = ActPARCorr, y = AllmeanTauAv, colour = as.factor(Phase)), size = 2, show.legend = T) +
  geom_line(aes(x = ActPARCorr, y = AllmeanTauAv, linetype = as.factor(Phase)), size = 0.3, show.legend = F) +
  #geom_point(aes(x = ActPARCorr, y = meanSig_nm2psii), colour = "darkgreen",size = 2, show.legend = T) +
  #geom_point(aes(x = ActPARCorr, y = AllmeanSig_nm2psii), colour = "red",size = 2, show.legend = T) +
    #scale_shape_manual(values = c(16, 1), name="", labels = lab2) +
  scale_fill_identity() +
...
SolFitsMetaLRC %>%
  ...
  filter(Ex_WL == 445 | Ex_WL == 535 | Ex_WL == 590) %>%
    filter(Phase!= "St") %>% 
  ggplot() +
  geom_point(aes(x = ActPARCorr, y = AllmeanqpOxbo, colour = as.factor(Phase)), size = 2, show.legend = T) +
  geom_line(aes(x = ActPARCorr, y = AllmeanqpOxbo, linetype = as.factor(Phase)), size = 0.3, show.legend = F) +
  #geom_point(aes(x = ActPARCorr, y = meanSig_nm2psii), colour = "darkgreen",size = 2, show.legend = T) +
  #geom_point(aes(x = ActPARCorr, y = AllmeanSig_nm2psii), colour = "red",size = 2, show.legend = T) +
    #scale_shape_manual(values = c(16, 1), name="", labels = lab2) +
  scale_fill_identity() +
...
SolFitsMetaLRC %>%
  ...
  filter(Ex_WL == 445 | Ex_WL == 535 | Ex_WL == 590) %>%
    filter(Phase!= "St") %>% 
  ggplot() +
  geom_point(aes(x = ActPARCorr, y = AllmeanJVPSII_umol_eLs, colour = as.factor(Phase)), size = 2, show.legend = T) +
  geom_line(aes(x = ActPARCorr, y = AllmeanJVPSII_umol_eLs, linetype = as.factor(Phase)), size = 0.3, show.legend = F) +
  #geom_point(aes(x = ActPARCorr, y = meanSig_nm2psii), colour = "darkgreen",size = 2, show.legend = T) +
  #geom_point(aes(x = ActPARCorr, y = AllmeanSig_nm2psii), colour = "red",size = 2, show.legend = T) +
    #scale_shape_manual(values = c(16, 1), name="", labels = lab2) +
  scale_fill_identity() +
...
```{r preliminary plot}
lab1=c(expression("PC-rich"), expression("PE-rich"))
lab2=c(expression("Exp"), expression("ExpResupply"))


# drectblue <- data.frame(Ex_WL = c("445", "445"), Strain = c("PC-rich", "PE-rich"), WL = c("450", "450"), Par_ue = c(60, 300), x1=c(-Inf), x2=c(Inf), y1=c(-Inf), y2=c(Inf))

drectgreen <- data.frame(Ex_WL = c("535", "535", "535", "535"), Strain = c("PC-rich", "PE-rich", "PC-rich", "PE-rich"), WL = c("530", "530"), Par_ue = c(60, 60, 300, 300),  x1=c(-Inf), x2=c(Inf), y1=c(-Inf), y2=c(Inf))

drectred <- data.frame(Ex_WL = c("590", "590"), Strain = c("PC-rich", "PE-rich"), WL = c("660", "660"), Par_ue = c(60, 60, 300, 300), x1=c(-Inf), x2=c(Inf), y1=c(-Inf), y2=c(Inf))
...
SolFitsMetaLRCTrim %>%
  ...
  geom_errorbar(aes(x = ActPARCorr, ymin = AllmeanSig_nm2psii - AllsdSig_nm2psii, ymax = AllmeanSig_nm2psii + AllsdSig_nm2psii, colour = as.factor(Phase)), width=0, size=0.3, show.legend = F) +
  ...
  geom_vline(xintercept = 30, linetype = "dotted", size = 0.7) +
  geom_vline(xintercept = 150, linetype = "dotted", size = 0.7) +
...
SolFitsMetaLRCTrim %>%
  ...
  geom_errorbar(aes(x = ActPARCorr, ymin = AllmeanTauAv - AllsdTauAv, ymax = AllmeanTauAv + AllsdTauAv, colour = as.factor(Phase)), width=0, size=0.3, show.legend = F) +
...
SolFitsMetaLRCTrim %>%
  ...
  geom_errorbar(aes(x = ActPARCorr, ymin = AllmeanqpOxbo - AllsdqpOxbo, ymax = AllmeanqpOxbo + AllsdqpOxbo, colour = as.factor(Phase)), width=0, size=0.3, show.legend = F) +
...
SolFitsMetaLRCTrimChoosen530<-SolFitsMetaLRCTrim %>% 
  filter(WL == 530) %>%
  filter(Ex_WL == 535)  
SolFitsMetaLRCTrimChoosen660<-SolFitsMetaLRCTrim %>% 
  filter(WL == 660) %>%
  filter(Ex_WL == 590) 
  
SolFitsMetaLRCTrimChoosen<-rbind(SolFitsMetaLRCTrimChoosen530, SolFitsMetaLRCTrimChoosen660)

rm(SolFitsMetaLRCTrimChoosen530, SolFitsMetaLRCTrimChoosen660)
...
data_textA <- data.frame(facetsExcitation  = c("Excitation~(nm)"), Par_ue = c(300), WL = c("660"), Strain  = c("PC-rich"), label = c('italic(a)'))
data_textB <- data.frame(facetsExcitation  = c("Excitation~(nm)"), Par_ue = c(300), WL = c("660"), Strain  = c("PC-rich"), label = c('italic(b)'))
data_textC <- data.frame(facetsExcitation  = c("Excitation~(nm)"), Par_ue = c(300), WL = c("660"), Strain  = c("PC-rich"), label = c('italic(c)'))
data_textD <- data.frame(facetsExcitation  = c("Excitation~(nm)"), Par_ue = c(300), WL = c("660"), Strain  = c("PC-rich"), label = c('italic(d)'))
...
Sig<-SolFitsMetaLRCTrimChoosen %>%
  ...
  geom_errorbar(aes(x = ActPARCorr, ymin = AllmeanSig_nm2psii - AllsdSig_nm2psii, ymax = AllmeanSig_nm2psii + AllsdSig_nm2psii, colour = as.factor(Phase)), width=0, size=0.3, show.legend = F) +
  geom_line(aes(x = ActPARCorr, y = AllmeanSig_nm2psii, linetype = as.factor(Phase)), size = 0.3, show.legend = T) +
  geom_vline(xintercept = 30, linetype = "dotted", size = 0.4) +
  geom_vline(xintercept = 150, linetype = "dotted", size = 0.4) +
...
Tau<-SolFitsMetaLRCTrimChoosen %>%
  mutate(WLNum = if_else(WL == "WW", 750, as.numeric(WL))) %>%
    filter(Phase!= "St") %>% 
  ggplot() +
  geom_point(aes(x = ActPARCorr, y = AllmeanTauAv, colour = as.factor(Phase)), size = 3, show.legend = F) +
...
qp<-SolFitsMetaLRCTrimChoosen %>%
  mutate(WLNum = if_else(WL == "WW", 750, as.numeric(WL))) %>%
    filter(Phase!= "St") %>% 
  ggplot() +
  geom_point(aes(x = ActPARCorr, y = AllmeanqpOxbo, colour = as.factor(Phase)), size = 3, show.legend = F) +
...
SolFitsExpStTurnerMD %>%
  ggplot() +
  geom_point(aes(x = ActPARCorr, y = AllmeanJVPSII_umol_eLs, colour = as.factor(Phase)), size = 2, show.legend = T) +
    geom_errorbar(aes(x = ActPARCorr, ymin = AllmeanJVPSII_umol_eLs - AllsdJVPSII_umol_eLs, ymax = AllmeanJVPSII_umol_eLs + AllsdJVPSII_umol_eLs, colour = as.factor(Phase)), width=0, size=0.3, show.legend = F) +
  ggh4x::facet_nested(cols = vars(WL, Par_ue), rows = vars(Strain), labeller = label_parsed) +
  theme_bw()
#colnames(SolFitsMetaLRC)
...
GrowthRateSol %>%
  filter(Phase != "St") %>% 
  ggplot() +
  geom_point(aes(x = ActPARCorr, y = JVPSII_umol_eumolChls, colour = as.factor(Phase)), size = 3, show.legend = T) +
