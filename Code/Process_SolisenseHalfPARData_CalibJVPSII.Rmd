...
#DataInCurve <- file.path("..", "Data", "ProcessedData", "ProcessedGrowthCurveData", fsep = .Platform$file.sep)
...
```
```{r}
# CurveFile <- "../Data/ProcessedData/ProcessedGrowthCurveData/BalticCCA_Processed_GrowthCurve.Rds"
# CurveFileName <- str_split(string = CurveFile, "/")[[1]][3] %>%
#   str_remove(pattern = ".Rds")
# CurveData <- readRDS(CurveFile)  %>%
#   ungroup()
```
...
# CurveData<-CurveData %>% 
#   select(-c(meanActinic_par_h, meanOD680_h, meanOD720_h, meanDeltaOD_h, ToD, Time_h, Tube, FilenameMC, ExpDate, ExpEndDate, facetsPar_ue, LightShape, SampleID)) %>% 
#   unique()
```
...
--------------------------------- Combine with df containing phase of growth -------------------
...
# SolFitsMetaPhase <- SolFitsMeta %>%
#   full_join(., CurveData, by = c("Run" = "Run", "Strain"="Strain",  "Par_ue"="Par_ue", "Photoperiod"="Photoperiod", "O2"="O2", "WL"="WL",  "E_days"="E_days")) %>%
#   drop_na(c(Time_h)) %>% 
#   unique() 
...
ChlDataMeta %>%
  ...
  ggh4x::facet_nested(rows = vars(Strain), cols = vars(Par_ue, Run),labeller = label_parsed) +
  theme_bw()
...
ChlDataMeta <- ChlDataMeta %>%
  group_by(Strain, WL, Par_ue, SampleID, Run,  E_days) %>%
  summarize(SampleID, Run, Strain, ExpDate, Par_ue, Photoperiod, O2, WL, PARPhotonDose_day, ObsDate, TurChl_ugL, E_days,
            meanTurChl_ugL = mean(TurChl_ugL),
            sdTurChl_ugL = sd(TurChl_ugL)) %>%
  ungroup() %>% 
  select(-c(ObsDate, ExpDate))
```
...
ChlDataMetaPhase %>%
  ggplot() +
  geom_point(aes(x = WL, y = TurChl_ugL)) +
  ggh4x::facet_nested(rows = vars(Strain), cols = vars(Par_ue, Run, Phase),labeller = label_parsed) +
  theme_bw()
...
# colnames(ChlDataMetaPhase)


SolFitsExpStTurner <- ChlDataMetaPhase %>%
  full_join(., SolFitsMetaLRC, by = c("SampleID"="SampleID", "Run"="Run", "Strain"="Strain",  "Par_ue"="Par_ue", "Photoperiod"="Photoperiod", 'O2'="O2", "WL"="WL", "PARPhotonDose_day"="PARPhotonDose_day", "Phase" = "Phase", "E_days"="E_days")) 
