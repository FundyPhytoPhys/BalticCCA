...
ChlDataMeta <- ChlDataMeta %>% # df contained mean chl data from only 3 technical replica - meanTurChl_ugL 
  select(-c(TurChl_ugL)) %>% 
  unique()
```
...
SolFitsMetaLRC<-SolFitsMetaLRC %>%
  select(-c(facetsExcitation, FilenameSolisense))

SolFitsExpStTurner <- SolFitsMetaLRC %>%
  full_join(., ChlDataMeta, by = c("SampleID"="SampleID", "Run"="Run", "Strain"="Strain", "Par_ue"="Par_ue", "Photoperiod"="Photoperiod", 'O2'="O2", "WL"="WL", "PARPhotonDose_day"="PARPhotonDose_day")) 
...
SolFitsExpStTurner %>%
  ggplot() +
  geom_point(aes(x = WL, y = JVPSII_umol_eLs)) +
  ggh4x::facet_nested(rows = vars(Strain), cols = vars(Par_ue, Run, Phase),labeller = label_parsed) +
  theme_bw()
...
SolFitsExpStTurner %>%
  filter(WL!= "WW") %>% 
  ggplot() +
  geom_point(aes(y = Sig, x = meanTurChl_ugL)) +
  ggh4x::facet_nested(rows = vars(Strain), cols = vars(Par_ue, Run),labeller = label_parsed) +
  theme_bw()
