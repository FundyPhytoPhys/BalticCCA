...
# Removing run 0 - it was needed to have a second growth rate after resupply
...
zip_file <- file.path("..", "Data", "RawData", "SolisenseSySlCCACaloxyBiolSteps.zip")
...
SolFitsTrimOS <- SolFitsOS %>%
  ...
  mutate(Filename = str_remove(string = Filename, pattern = "/RawData/SolisenseSySlCCACaloxyBiolSteps.zip/"),
         Filename = str_remove(string = Filename, pattern = "_fit.csv")
         ) %>%
  separate(Filename, into = c("fp1", "fp2", "fp3", "fp4", "fp5", "fp6", "fp7", "Project", "RunDateTime", "Method", "SampleID", "Ex_WL"), sep = "([\\/\\_])", remove = FALSE) %>%
  ...
  select(-c(Light_1,Light_2,Light_3,Light_4,Light_5,Light_6,`Source DataFile`,DATE,TIME,RunDateTime, fp1, fp2, fp3, fp4, fp5, fp6, fp7, Project, Method))
...
SolFitsMeta <- SolFitsMeta %>%
  group_by(SourceDataFile, Filename, SampleID, ObsDate, Ex_WL, TempCont, WL) %>%
  mutate(Sig_m2psii = Sig/A2_m2,
         Fodark = Fo[1],
         Fmdark = Fm[1],
         Sigdark = Sig[1],
         
         Tau1QAdark = Tau1QA[1], #SySl CCA
         Tau1QAmin = min(Tau1QA, na.rm = TRUE), #SySl CCA
         
         Tau3QAdark = Tau3QA[1], #SySl CCA
         Tau3QAmin = min(Tau3QA, na.rm = TRUE), #SySl CCA
         ...
         TauAvdark = TauAv[1], #SySl CCA
         TauAvmin = min(TauAv, na.rm = TRUE), #SySl CCA 
...
# Convert Sig_m2psii to Sig_nm2psii and Sigdark_m2psii to Sigdark_nm2psii and calculated number of days when measurements were made - mole to nanomole
...
SolFitsMeta %>%
  ...
  ggplot() +
  geom_point(aes(x = Par_ue, y = JVPSII_ETRqpOxbo_FoSig, colour = as.factor(Strain)), size = 3.5, show.legend = T) +
  scale_colour_discrete(type=c("palegreen3", "brown4"), name="", labels = lab1) +
...
saveRDS(SolFitsMeta, file.path(DataOut, paste(Project, "Imported_SolisenseLight_LRC_CaloxyBiolSteps.Rds", sep = "_"), fsep = .Platform$file.sep), ascii = FALSE, version = NULL, compress = "xz", refhook = NULL)
