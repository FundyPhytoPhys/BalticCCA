...
rm(SolFitsOS, SolFitsTrimOS, SolFitsTrimMetaOS, SolFitsTrimMetaOS_param, SolFitsTrimOS_lightsteps, CultureCatalog, ActPARCrossCal)
```
...
```{r}
SolFitsMeta <- SolFitsTrimMetaCleanOS %>% 
  rename(FilenameSolisense=Filename)

SolFitsMetaDarkafterLight <-SolFitsTrimMetaDarkafterLightOS %>% 
  rename(FilenameSolisense=Filename)

rm(SolFitsTrimMetaCleanOS, SolFitsTrimMetaDarkafterLightOS)
```
...
SolFitsMeta %>%
  ...
  facet_grid(rows = vars(Strain), cols = vars(WL, Par_ue)) +
  theme_bw()
...
```{r save plot}
ggsave(file = file.path(PlotsPath, paste("Solisense_SupPlot",".png",sep = "")), height=10, width= 8,  dpi = 300, limitsize = TRUE)
```
