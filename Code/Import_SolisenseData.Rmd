...
#   left_join(., SolisenseAll, by = c("SampleID" = "SampleID")) 
...
SolFitsMeta %>%
  ...
  facet_grid(rows = vars(Ex_WL), cols = vars(WL)) +
  theme_bw()
# 
...
```{r read locally stored metadata from rds}
SolisenseAll <- readRDS(file = file.path("..", "Data","ImportedData", "ImportedSolisenseData", "BalticO2_Imported_SolFitsTrimOS_lightstepsAll.Rds"))

SolisenseAll<-SolisenseAll %>% 
  select(-c(PrimaryOperator, Temp_c, ExpCul, ExpStartTime, O2_Category, Optode, OptodeCh, OptodeMeasure))
```
