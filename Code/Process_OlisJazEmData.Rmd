...
# For plot only - Only run 70 - exp and resupply
# Run 56 after day 7 also sowed change in pigments without resupply!
 
```{r}

OLISSpectraMetaPUR_Resup2<-OLISSpectraMetaPUR %>%
  filter(WL!= "WW") %>% 
  filter(Run == 70 | Run == 68 | Run == 63) %>% 
 filter(E_days == 2 | E_days == 16)


OLISSpectraMetaPUR_Resup2<-OLISSpectraMetaPUR_Resup2 %>% 
  mutate(Phase=case_when(E_days<16~"Exp",
         E_days>=16~"ExpResupply"))
...
```{r}
OLISSpectraMetaPUR_Resup2$facetsPar_ue = factor(OLISSpectraMetaPUR_Resup2$O2, labels = c("PAR~(µmol~photons~m^{-2}~s^{-1})"))
OLISSpectraMetaPUR_Resup2$facetsPhotoperiod = factor(OLISSpectraMetaPUR_Resup2$O2, labels = c("Photoperiod~(h)"))
OLISSpectraMetaPUR_Resup2$facetsStrain = factor(OLISSpectraMetaPUR_Resup2$O2, labels = c("Strain"))
```
...
data_textExp <- data.frame(Strain  = c("PC-rich"), Par_ue  = c(60, 300), label = c('Exp'))
data_textResup <- data.frame(Strain  = c("PC-rich"), Par_ue  = c(60, 300), label = c('Resupply'))
...
lab2=c(expression("530"), expression("660"))

OLISSpectraMetaPUR_Resup2 %>%
  # filter(WL!= "WW") %>% 
  # filter(Run == 70 | Run == 68) %>% 
  #filter(Time_h <= 192) %>% 
  ggplot() +
  
  #geom_area(aes(x = Time_h, y = meanActinic_par_h), size = 0.1, fill = "tan1", alpha = 0.6) +
    geom_line(aes(x = nm, y = AbsNorm440, colour = as.factor(WL)), show.legend = T, size = 0.4) +
  # geom_vline(aes(xintercept = 13.5), linetype = "dashed") +
  
  # geom_text(data=data_textExp, aes(x=6, y=1.2, label=label), size=4) +
  # geom_text(data=data_textResup, aes(x=17, y=1.2, label=label), size=4) +
  ...
  #scale_colour_discrete(type=c("black", "deepskyblue2"), name="", labels = lab1) +
  ...
  stat_wl_strip(aes(x = nm), ymin = -Inf, ymax = 0, alpha = 0.5) +
  scale_fill_identity() +
  
  ggh4x::facet_nested(rows = vars(Strain), cols = vars(Phase, Par_ue),labeller = labeller(Ex_WL = label_value, WL = label_value)) +
...
```{r}
OLISSpectraMetaPUR_Resup2<-OLISSpectraMetaPUR_Resup2 %>%     
mutate(WLnm=case_when(WL==530~"530~nm",
    WL==660~"660~nm"))  
```
...
# Create GrowthCurve plot
fig.height = 8, fig.width = 8, warning = FALSE
```{r reate plot, fig.height = 5, fig.width = 7, warning = FALSE}
...
OLISSpectraMetaPUR_Resup2 %>%
  ...
  #geom_area(aes(x = Time_h, y = meanActinic_par_h), size = 0.1, fill = "tan1", alpha = 0.6) +
    geom_line(aes(x = nm, y = AbsNorm440, colour = as.factor(Phase)), show.legend = T, size = 0.4) +
  # geom_vline(aes(xintercept = 13.5), linetype = "dashed") +
  ...
  #scale_colour_discrete(type=c("palegreen3", "brown4")) +
  #scale_colour_discrete(type=c("black", "deepskyblue2"), name="", labels = lab1) +
  scale_colour_manual(values = c("slateblue4", "skyblue3")) +
  # scale_linetype_manual(values = c("solid", "dashed"), name="", labels = lab1) +
  #scale_colour_manual(values = c("palegreen3", "brown4"), name="", labels = lab1) +
  labs(y = "Normalized whole-cell absorbance", x = "Wavelength (nm)") +
  ...
  ggh4x::facet_nested(rows = vars(facetsStrain, Strain), cols = vars(facetsPar_ue, Par_ue, WLnm),labeller = label_parsed) +
...
# Filter 8 spectra that are weird - day 7. I will deal with it later. I think this could be contamination?
...
# Create plot - all spectra I made for each run
...
MultiCultiDataAll70ExpResup<-OLISSpectraMetaPUR %>% 
  filter(E_days >= 14)  
MultiCultiDataAll70ExpResup$Phase<-"ExpResupply"
...
# Create prelim plot with all PUR
```{r}
...
OLISSpectraMetaPhase %>%
  filter(Phase!= "St") %>% 
  ggplot() +
  geom_point(aes(x = WL, y = PUR, colour = as.factor(Phase)), show.legend = T) +
  ggh4x::facet_nested(rows = vars(Strain), cols = vars(Par_ue),labeller = labeller(Ex_WL = label_value, WL = label_value)) +
  theme_bw() 
...
OLISSpectraMetaPhase %>%
  #filter(Phase!= "St") %>% 
  ggplot() +
  geom_point(aes(x = E_days, y = PUR/Par_ue, colour = as.factor(Phase)), show.legend = T) +
  ggh4x::facet_nested(rows = vars(Strain), cols = vars(Par_ue, WL),labeller = labeller(Ex_WL = label_value, WL = label_value)) +
  theme_bw() 
```



# Create a better plot - only exp i ExpResup - spectra taken at E_day = 2 and all PUR data
...
OLISSpectraMetaPhase %>%
 ...
 filter(E_days == 2 | E_days == 16) %>% 
  ggplot() +
  geom_point(aes(x = WL, y = PUR, colour = as.factor(Phase)), size = 4, show.legend = T) +
  ggh4x::facet_nested(rows = vars(Strain), cols = vars(Par_ue),labeller = labeller(Ex_WL = label_value, WL = label_value)) +
  theme_bw() 
...
OLISSpectraMetaPhase %>%
 ...
 filter(E_days == 2 | E_days == 16) %>% 
  ggplot() +
  geom_point(aes(x = Par_ue, y = PUR, colour = as.factor(Phase)), size = 4, show.legend = T) +
    geom_smooth(aes(x = Par_ue, y = PUR, linetype=as.factor(Phase)), size = 1, alpha = 0.2, show.legend = T, method = "lm") +
  ggh4x::facet_nested(rows = vars(Strain), cols = vars(WL),labeller = labeller(Ex_WL = label_value, WL = label_value)) +
  theme_bw() 
...
OLISSpectraMetaPhase %>%
  ...
  ggplot() +
  geom_point(aes(x = Par_ue, y = PUR, colour = as.factor(Phase)), size = 4, show.legend = T) +
    geom_smooth(aes(x = Par_ue, y = PUR, linetype=as.factor(Phase)), colour = "black", size = 0.7, alpha = 0.2, show.legend = T, method = "lm") +
  scale_colour_manual(values = c("slateblue4", "skyblue3")) +
  ggh4x::facet_nested(rows = vars(Strain), cols = vars(WL),labeller = labeller(Ex_WL = label_value, WL = label_value)) +
  theme_bw()
```

# For plot only - Create a numeric WL 

```{r}
OLISSpectraMetaPhasePlot<-OLISSpectraMetaPhase %>% 
  mutate(WLNum = if_else(WL == "WW", 750, as.numeric(WL))) %>%
  filter(WL != "WW") 
```

# For plot only - choosen only run 70 where Exp and ExpResupl was measured

```{r}
OLISSpectraMetaPhasePlot <- OLISSpectraMetaPhasePlot %>%
  filter(Phase!= "St") %>% 
  filter(WL!= "WW") %>% 
  filter(Run == 70 | Run == 68 | Run == 63) %>% 
 filter(E_days == 2 | E_days == 16) 
...
```{r}
OLISSpectraMetaPhasePlot$facetsPar_ue = factor(OLISSpectraMetaPhasePlot$O2, labels = c("PAR~(µmol~photons~m^{-2}~s^{-1})"))
OLISSpectraMetaPhasePlot$facetsPhotoperiod = factor(OLISSpectraMetaPhasePlot$O2, labels = c("Photoperiod~(h)"))
OLISSpectraMetaPhasePlot$facetsStrain = factor(OLISSpectraMetaPhasePlot$O2, labels = c("Strain"))
```

# Create plot

```{r create plot, fig.height = 5, fig.width = 6, warning = FALSE}

lab1=c(expression("PC-rich"), expression("PE-rich"))
lab2=c(expression("Exp"), expression("ExpResupply"))


OLISSpectraMetaPhasePlot %>%
  ggplot() +
  #geom_area(aes(x = nm, y = EmNormJaz722, fill = "indianred4"), alpha = 0.1, show.legend = F) +
  geom_area(aes(x = nm, y = EmNormJaz647*100, fill = "brown1"), alpha = 0.1, show.legend = F) +
  #geom_area(aes(x = nm, y = EmNormJaz612, fill = "coral"), alpha = 0.1, show.legend = F) +
  geom_area(aes(x = nm, y = EmNormJaz521*100, fill = "limegreen"), alpha = 0.1, show.legend = F) +
  ...
  geom_point(aes(x = WLNum, y = PUR, colour = as.factor(Phase)), size = 4, show.legend = T) +
  geom_line(aes(x = WLNum, y = PUR, linetype = as.factor(Phase)), show.legend = F) +
  scale_colour_manual(values = c("slateblue4", "skyblue3")) +
  # geom_errorbar(aes(x = WLNum, ymin = deltaOD_Lmu_corr - deltaOD_Lmu_se, ymax = deltaOD_Lmu_corr + deltaOD_Lmu_se), width=0, size=0.3, show.legend = F, OLISSpectraMetaPhasePlot) +
  
  # scale_colour_manual(values = c("palegreen3", "brown4"), name="", labels = lab2) +
  # scale_shape_manual(values = c(16, 18), name="", labels = lab2) +
  # scale_linetype_manual(values = c("solid", "dashed"), name="", labels = lab2) +
  
  stat_wl_strip(aes(x = nm), ymin = -Inf, ymax = -5, alpha = 0.5) +
  scale_fill_identity() +
  labs(y = "PUR and LED emission spectra", x = "Growth waveband (nm)") +
  ggh4x::facet_nested(rows = vars(facetsStrain, Strain), cols = vars(facetsPar_ue, Par_ue), labeller = label_parsed) +
  theme_bw() +
  theme(panel.grid.minor = element_blank(),
        panel.grid.major = element_blank(),
        #panel.spacing.x = unit(0.4, 'cm'),
...
# Mean from the absorbance and PUR and calculate PUR/PAR ratio

# I filter day = 0 and day = 7 to avoid data from very early and very late exp phase
...
# Checking spectra in different phases
...
```{r}
OLISSpectraMetaPhaseSpectraPlot70<-OLISSpectraMetaPhase %>% 
  filter(Run == 70) %>% 
  filter(Phase !="Exp")


OLISSpectraMetaPhaseSpectraPlot60<-OLISSpectraMetaPhase %>% 
    filter(Run == 63) %>% 
  filter(Phase =="Exp") %>% 
  filter(Par_ue == 60) %>% 
  filter(E_days !=13)

OLISSpectraMetaPhaseSpectraPlot300<-OLISSpectraMetaPhase %>% 
    filter(Run == 63) %>% 
  filter(Phase =="Exp") %>% 
  filter(Par_ue == 300) 

OLISSpectraMetaPhaseSpectraPlotst<-OLISSpectraMetaPhase %>% 
  filter(Phase =="St")  

OLISSpectraMetaPhaseSpectraPlot <-rbind(OLISSpectraMetaPhaseSpectraPlot70, OLISSpectraMetaPhaseSpectraPlot60, OLISSpectraMetaPhaseSpectraPlot300, OLISSpectraMetaPhaseSpectraPlotst)

```


```{r}
OLISSpectraMetaPhaseSpectraPlot %>%
  #filter(Run == 63 | Run == 70 | Run == 56) %>% 
    filter(E_days != 11) %>%
  ggplot() +
  geom_line(aes(x = nm, y = AbsNorm440, colour = as.factor(E_days)), show.legend = T) +
  ggh4x::facet_nested(rows = vars(Strain, Phase), cols = vars(Par_ue, WL),labeller = labeller(Ex_WL = label_value, WL = label_value)) +
  theme_bw() 
...
```{r }
OLISSpectraMetaPhaseSpectraPlot %>%
  filter(E_days != 11 & E_days != 10) %>%
  ggplot() +
  geom_line(aes(x = nm, y = AbsNorm440, colour = as.factor(Phase)), show.legend = F) +
  scale_colour_manual(values = c("slateblue4",  "skyblue3", "lightblue3"), name="", labels = lab2) +
  ggh4x::facet_nested(rows = vars(Strain, Phase), cols = vars(Par_ue, WL),labeller = labeller(Ex_WL = label_value, WL = label_value)) +
  theme_bw() 

```
```{r}
OLISSpectraMetaPhaseSpectraPlot$facetsPar_ue = factor(OLISSpectraMetaPhaseSpectraPlot$O2, labels = c("PAR~(µmol~photons~m^{-2}~s^{-1})"))
OLISSpectraMetaPhaseSpectraPlot$facetsPhotoperiod = factor(OLISSpectraMetaPhaseSpectraPlot$O2, labels = c("Photoperiod~(h)"))
OLISSpectraMetaPhaseSpectraPlot$facetsStrain = factor(OLISSpectraMetaPhaseSpectraPlot$O2, labels = c("Strain"))
```


```{r create plot, fig.height = 8, fig.width = 6, warning = FALSE}
lab1=c(expression("PC-rich"), expression("PE-rich"))
lab2=c(expression("530 nm"), expression("660 nm"))

data_textpigmentPC <- data.frame(Par_ue = c(60, 300, 60, 300, 60, 300), facetsPar_ue = c("PAR~(µmol~photons~m^{-2}~s^{-1})"), facetsStrain = c("Strain"), Strain  = c("PC-rich", "PC-rich", "PC-rich"), Phase  = c("Exp", "ExpResupply", "St"), label = c('PC'))

data_textpigmentPC2 <- data.frame(Par_ue = c(60, 300), facetsPar_ue = c("PAR~(µmol~photons~m^{-2}~s^{-1})"), facetsStrain = c("Strain"), Strain  = c("PE-rich"), Phase  = c("ExpResupply"), label = c('PC'))

data_textpigmentPE <- data.frame(Par_ue = c(60, 300, 60, 300, 60, 300), facetsPar_ue = c("PAR~(µmol~photons~m^{-2}~s^{-1})"), facetsStrain = c("Strain"), Strain  = c("PE-rich", "PE-rich", "PE-rich"), Phase  = c("Exp", "ExpResupply", "St"), label = c('PE'))

data_textpigmentPE2 <- data.frame(Par_ue = c(60, 300), facetsPar_ue = c("PAR~(µmol~photons~m^{-2}~s^{-1})"), facetsStrain = c("Strain"), Strain  = c("PC-rich"), Phase  = c("ExpResupply"), label = c('PE'))

arrowdf1 <- tibble(facetsPar_ue = "PAR~(µmol~photons~m^{-2}~s^{-1})", Par_ue = c(60, 300), facetsStrain = "Strain", Strain = "PC-rich", Phase = "Exp")
arrowdf2 <- tibble(facetsPar_ue = "PAR~(µmol~photons~m^{-2}~s^{-1})", Par_ue = c(60, 300), facetsStrain = "Strain", Strain = "PC-rich", Phase = "ExpResupply")
arrowdf3 <- tibble(facetsPar_ue = "PAR~(µmol~photons~m^{-2}~s^{-1})", Par_ue = c(60, 300), facetsStrain = "Strain", Strain = "PC-rich", Phase = "St")
arrowdf4 <- tibble(facetsPar_ue = "PAR~(µmol~photons~m^{-2}~s^{-1})", Par_ue = c(60, 300), facetsStrain = "Strain", Strain = "PE-rich", Phase = "ExpResupply")

arrowdf5 <- tibble(facetsPar_ue = "PAR~(µmol~photons~m^{-2}~s^{-1})", Par_ue = c(60, 300), facetsStrain = "Strain", Strain = "PE-rich", Phase = "Exp")
arrowdf6 <- tibble(facetsPar_ue = "PAR~(µmol~photons~m^{-2}~s^{-1})", Par_ue = c(60, 300), facetsStrain = "Strain", Strain = "PE-rich", Phase = "ExpResupply")
arrowdf7 <- tibble(facetsPar_ue = "PAR~(µmol~photons~m^{-2}~s^{-1})", Par_ue = c(60, 300), facetsStrain = "Strain", Strain = "PE-rich", Phase = "St")
arrowdf8 <- tibble(facetsPar_ue = "PAR~(µmol~photons~m^{-2}~s^{-1})", Par_ue = c(60, 300), facetsStrain = "Strain", Strain = "PC-rich", Phase = "ExpResupply")


OLISSpectraMetaPhaseSpectraPlot %>%
  #filter(E_days != 11 & E_days != 10) %>%
    filter(E_days == 2 | E_days == 13 | E_days == 16) %>%
  ggplot() +
  geom_line(aes(x = nm, y = AbsNorm440, colour = as.factor(WL)), show.legend = T) +
  scale_colour_manual(values = c("palegreen4",  "brown4"), name="", labels = lab2) +
  
  geom_text(data=data_textpigmentPC, aes(x=660, y=1.4, label=label), size=3, colour = "deepskyblue4") + #PC
  geom_text(data=data_textpigmentPC2, aes(x=660, y=1.4, label=label), size=3, colour = "deepskyblue4") + #PC
  
  geom_text(data=data_textpigmentPE, aes(x=510, y=1.4, label=label), size=3, colour = "hotpink4") + #PE
  geom_text(data=data_textpigmentPE2, aes(x=510, y=1.4, label=label), size=3, colour = "hotpink4") + #PE
  
  geom_segment(data = arrowdf1,
               aes(x = 665, xend = 645, y = 1.25, yend = 1.0),
  ...
  geom_segment(data = arrowdf3,
               aes(x = 665, xend = 645, y = 1.25, yend = 0.6),
               colour = "deepskyblue4", size = 0.5, alpha=0.9, arrow = arrow(length=unit(0.2, 'cm'))) + #PC
  geom_segment(data = arrowdf4,
               aes(x = 665, xend = 645, y = 1.25, yend = 0.9),
               colour = "deepskyblue4", size = 0.5, alpha=0.9, arrow = arrow(length=unit(0.2, 'cm'))) + #PC
  
  
  geom_segment(data = arrowdf5,
               aes(x = 510, xend = 530, y = 1.25, yend = 1.0),
               colour = "hotpink4", size = 0.5, alpha=0.9, arrow = arrow(length=unit(0.2, 'cm'))) + #PE
  geom_segment(data = arrowdf6,
               aes(x = 510, xend = 530, y = 1.25, yend = 0.9),
               colour = "hotpink4", size = 0.5, alpha=0.9, arrow = arrow(length=unit(0.2, 'cm'))) + #PE
  geom_segment(data = arrowdf7,
               aes(x = 510, xend = 535, y = 1.25, yend = 0.6),
               colour = "hotpink4", size = 0.5, alpha=0.9, arrow = arrow(length=unit(0.2, 'cm'))) + #PE
  ...
  coord_cartesian(ylim = c(0, 1.5)) +
    ...
    labs(y = "Normalized whole-cell absorbance", x = "Wavelength (nm)") +
  ggh4x::facet_nested(rows = vars(facetsStrain, Strain, factor(Phase, levels=c("Exp", "St", "ExpResupply"))), cols = vars(facetsPar_ue, Par_ue),labeller = label_parsed) +
...
```{r}
OLISSpectraMetaPhasemeanPlot<-OLISSpectraMetaPhasemean %>% 
  mutate(WLNum = if_else(WL == "WW", 750, as.numeric(WL))) %>%
  filter(WL != "WW") 
```
...
```{r}
OLISSpectraMetaPhasemeanPlot$facetsPar_ue = factor(OLISSpectraMetaPhasemeanPlot$O2, labels = c("PAR~(µmol~photons~m^{-2}~s^{-1})"))
OLISSpectraMetaPhasemeanPlot$facetsPhotoperiod = factor(OLISSpectraMetaPhasemeanPlot$O2, labels = c("Photoperiod~(h)"))
OLISSpectraMetaPhasemeanPlot$facetsStrain = factor(OLISSpectraMetaPhasemeanPlot$O2, labels = c("Strain"))
```


```{r}
OLISSpectraMetaPhasemeanPlot60<-OLISSpectraMetaPhasemeanPlot %>% 
  filter(Par_ue == 60) %>% 
  mutate(EmNormJaz647_60 = EmNormJaz647*60) %>% 
  mutate(EmNormJaz521_60 = EmNormJaz521*60) 


OLISSpectraMetaPhasemeanPlot300<-OLISSpectraMetaPhasemeanPlot %>% 
  filter(Par_ue == 300) %>% 
  #filter(WL == 530 | WL == 660) %>% 
  mutate(EmNormJaz647_300 = EmNormJaz647*300) %>% 
  mutate(EmNormJaz521_300 = EmNormJaz521*300) 
...
OLISSpectraMetaPhasemeanPlot %>%
  filter(Phase!= "St") %>% 
  ggplot() +
  #geom_area(aes(x = nm, y = EmNormJaz722, fill = "indianred4"), alpha = 0.1, show.legend = F) +
  geom_area(aes(x = nm, y = EmNormJaz647*60, fill = "brown1"), alpha = 0.1, show.legend = F, OLISSpectraMetaPhasemeanPlot60) +
  #geom_area(aes(x = nm, y = EmNormJaz612, fill = "coral"), alpha = 0.1, show.legend = F) +
  geom_area(aes(x = nm, y = EmNormJaz521*60, fill = "limegreen"), alpha = 0.1, show.legend = F, OLISSpectraMetaPhasemeanPlot60) +
  
    geom_area(aes(x = nm, y = EmNormJaz647*300, fill = "brown1"), alpha = 0.1, show.legend = F, OLISSpectraMetaPhasemeanPlot300) +
  #geom_area(aes(x = nm, y = EmNormJaz612, fill = "coral"), alpha = 0.1, show.legend = F) +
  geom_area(aes(x = nm, y = EmNormJaz521*300, fill = "limegreen"), alpha = 0.1, show.legend = F, OLISSpectraMetaPhasemeanPlot300) +
  ...
  geom_point(aes(x = WLNum, y = meanPUR, colour = as.factor(Phase)), size = 4, show.legend = T) +
  geom_line(aes(x = WLNum, y = meanPUR, linetype = as.factor(Phase)), show.legend = F) +
  scale_colour_manual(values = c("slateblue4", "skyblue3")) +
  geom_errorbar(aes(x = WLNum, ymin = meanPUR - sdPUR, ymax = meanPUR + sdPUR), width=0, size=0.3, show.legend = F) +
...
OLISSpectraMetaPhasemeanPlot %>%
  filter(Phase!= "St") %>% 
    filter(WL!= "WW") %>% 
  ggplot() +
  geom_point(aes(x = Par_ue, y = meanPUR, colour = as.factor(Phase)), size = 4, show.legend = T) +
    geom_smooth(aes(x = Par_ue, y = meanPUR, linetype=as.factor(Phase)), colour = "black", size = 0.7, alpha = 0.2, show.legend = T, method = "lm") +
    geom_errorbar(aes(x = Par_ue, ymin = meanPUR - sdPUR, ymax = meanPUR + sdPUR), width=0, size=0.3, show.legend = F) +
  scale_colour_manual(values = c("slateblue4", "skyblue3")) +
  ggh4x::facet_nested(rows = vars(Strain), cols = vars(WL),labeller = labeller(Ex_WL = label_value, WL = label_value)) +
  theme_bw() +
    theme(panel.grid.minor = element_blank(),
...
OLISAllPURTidy %>%
  ggplot() +
  geom_point(aes(x = WL, y = meanPURPARRatio, colour = as.factor(Strain)), alpha = 0.9, size = 3, show.legend = T) +
