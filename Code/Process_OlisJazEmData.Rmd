...
OLISSpectraMetaPUR %>%
  ...
  geom_line(aes(x = nm, y = AbsNorm440, colour = as.factor(Strain)), show.legend = T, size = 0.4) +
  #scale_color_manual(values = c("PC-rich" ="palegreen3", "PE-rich" ="brown4")) +
  ggh4x::facet_nested(rows = vars(Strain), cols = vars(WL, Par_ue, E_days),labeller = labeller(Ex_WL = label_value, WL = label_value)) +
  # ggh4x::facet_nested(cols = vars(WL),   rows = vars(factor(O2, levels=c("21","0"))),labeller = labeller(Ex_WL = label_value, WL = label_value)) +
  theme_bw() 
...
# Mean from the absorbance - issue - filter only exp data!!!!
# more issue - think about mean!! Maybe filter data where no difference at the early exp phase
...
OLISSpectraMetaPURmean<-OLISSpectraMetaPUR %>% 
  #filter(E_days == 7 | E_days == 6) %>% 
  group_by(Strain, Par_ue, WL, O2, nm) %>%
  
  summarize(SampleID, Run, Strain, ExpDate, Par_ue, Photoperiod, O2, WL,  PARPhotonDose_day, PURPhotonDose_day, ObsDate, Absorbance, nm, E_days, AbsNorm440, EmNormJaz722, EmNormJaz600, EmNormJaz647, EmNormJaz612, EmNormJaz521, EmNormJaz469, EmNormJaz441, EmNormJaz393, PUR, PURNorm, 
...
OLISSpectraMetaPURmean<-OLISSpectraMetaPURmean %>%
...
OLISSpectraMetaPURmean %>%
  ...
  scale_y_continuous(breaks=seq(0, 1.2, by = 0.4)) +
  coord_cartesian(ylim = c (-0.01, 1.6)) +
  ggh4x::facet_nested(rows = vars(Strain, E_days), cols = vars(WL, Par_ue),labeller = labeller(Ex_WL = label_value, WL = label_value)) +
  theme_bw() 
...
Spectra<-OLISSpectraMetaPURmean %>%
  ...
  geom_line(aes(x = nm, y = meanPURNorm), size = 0.5, show.legend = F) +
  ...
  geom_line(aes(x = nm, y = AbsNorm440, linetype = as.factor(WL)), show.legend = F, size = 0.4) +
...
OLISAllPURTidy<-OLISSpectraMetaPURmean %>% 
  select(c(SampleID, Run, Strain, ExpDate, Par_ue, Photoperiod, O2, WL, PARPhotonDose_day, PURPhotonDose_day, PUR, meanPUR, sdPUR, E_days, PURPAR)) %>% 
  unique() %>% 
