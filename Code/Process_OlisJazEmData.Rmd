...
list.files(path = DataInCurve, pattern = Project, full.names = TRUE)
```
...
CurveData<-CurveData %>% 
  select(-c(meanActinic_par_h, meanOD680_h, meanOD720_h, meanDeltaOD_h, ToD, Time_h, Tube, FilenameMC, ExpDate, ExpEndDate, facetsPar_ue, LightShape)) %>% 
  unique()
```
...
OLISSpectraMetaPhase <- OLISSpectraMetaPUR %>%
  full_join(., CurveData, by = c("SampleID" = "SampleID", "Run" = "Run", "Strain"="Strain",  "Par_ue"="Par_ue", "Photoperiod"="Photoperiod", "O2"="O2", "WL"="WL",  "E_days"="E_days")) %>%
  drop_na(c(PUR,  Phase)) %>% 
  unique() 
