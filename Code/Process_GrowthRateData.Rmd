...
# Separate to phases - Growth rate doesn't have stationary phase!!!

```{r}
MultiCultiGrowth<-MultiCultiGrowth %>% 
  mutate(Phase=case_when(Run==0~"ExpResupply",
         Run==56~"Exp", Run==59~"Exp", Run==63~"Exp", Run==68~"Exp", Run==70~"Exp"))
```
...
```{r}
MultiCultiDataAll1<-MultiCultiGrowth %>% 
  filter(Strain == "PE-rich") 


MultiCultiDataAll2<-MultiCultiGrowth %>% 
  filter(Strain == "PC-rich") %>% 
  filter(Run != 56)

MultiCultiGrowth<-rbind(MultiCultiDataAll1, MultiCultiDataAll2)

rm(MultiCultiDataAll1, MultiCultiDataAll2)
```
...
MultiCultiGrowth %>%
  ...
  ggplot() +
  geom_point(aes(x = WL, y = deltaOD_Lmu_corr,colour=as.factor(Run)), size = 3.5, alpha = 0.9, show.legend = T) +
  #geom_line(aes(x = WL, y = deltaOD_Lmu_corr, linetype=as.factor(Strain)), size = 1, alpha = 0.9, show.legend = T) +
  # geom_errorbar(aes(x = Par_ue, ymin = deltaOD_Lmu_corr - deltaOD_Lmu_se, ymax = deltaOD_Lmu_corr + deltaOD_Lmu_se), width=0, size=0.3, show.legend = F) +
...
MultiCultiGrowth %>%
  filter(deltaOD_Lmu_corr > 0) %>% 
  ggplot() +
  geom_point(aes(x = Par_ue, y = deltaOD_Lmu_corr,colour=as.factor(WL)), size = 3.5, alpha = 0.9, show.legend = T) +
    geom_smooth(aes(x = Par_ue, y = deltaOD_Lmu_corr, linetype=as.factor(WL)), size = 1, alpha = 0.2, show.legend = T, method = "lm") +
  #geom_line(aes(x = WL, y = deltaOD_Lmu_corr, linetype=as.factor(Strain)), size = 1, alpha = 0.9, show.legend = T) +
  # geom_errorbar(aes(x = Par_ue, ymin = deltaOD_Lmu_corr - deltaOD_Lmu_se, ymax = deltaOD_Lmu_corr + deltaOD_Lmu_se), width=0, size=0.3, show.legend = F) +
    ggh4x::facet_nested(rows = vars(Strain),labeller = labeller(Ex_WL = label_value, WL = label_value)) +
  theme_bw()
...
MultiCultiGrowth %>%
  ...
  filter(Run == 70 | Run == 0) %>% 
  ggplot() +
  geom_point(aes(x = WL, y = deltaOD_Lmu_corr,colour=as.factor(Phase)), size = 3.5, alpha = 0.9, show.legend = T) +
  #geom_line(aes(x = WL, y = deltaOD_Lmu_corr, linetype=as.factor(Phase)), size = 1, alpha = 0.9, show.legend = T) +
  # geom_smooth(aes(x = WL, y = deltaOD_Lmu_corr, linetype=as.factor(Phase)), size = 1, alpha = 0.2, show.legend = T, method = "lm") +
  geom_errorbar(aes(x = WL, ymin = deltaOD_Lmu_corr - deltaOD_Lmu_se, ymax = deltaOD_Lmu_corr + deltaOD_Lmu_se), width=0, size=0.3, show.legend = F) +
    ggh4x::facet_nested(rows = vars(Strain),labeller = labeller(Ex_WL = label_value, WL = label_value)) +
    ggh4x::facet_nested(rows = vars(Strain), cols = vars(Par_ue),labeller = labeller(Ex_WL = label_value, WL = label_value)) +
  theme_bw()
...
# lab1=c(expression("530"), expression("660"))
# 
# MultiCultiGrowth %>%
#   filter(deltaOD_Lmu_corr > 0) %>% 
#   filter(Run == 70 | Run == 0) %>% 
...
#     scale_linetype_manual(values = c("dotted", "solid"), name="", labels = lab1) +
#   
#   ggh4x::facet_nested(rows = vars(Strain), cols = vars(Phase), labeller = labeller(Ex_WL = label_value, WL = label_value)) +
#   theme_bw()
...
MultiCultiGrowth %>%
  ...
  filter(Run == 70 | Run == 0) %>% 
  ggplot() +
  geom_point(aes(x = Par_ue, y = deltaOD_Lmu_corr,colour=as.factor(Phase)), size = 3.5, alpha = 0.9, show.legend = T) +
    geom_smooth(aes(x = Par_ue, y = deltaOD_Lmu_corr, linetype=as.factor(Phase)), size = 1, alpha = 0.2, show.legend = T, method = "lm") +
  #geom_line(aes(x = WL, y = deltaOD_Lmu_corr, linetype=as.factor(Strain)), size = 1, alpha = 0.9, show.legend = T) +
  geom_errorbar(aes(x = Par_ue, ymin = deltaOD_Lmu_corr - deltaOD_Lmu_se, ymax = deltaOD_Lmu_corr + deltaOD_Lmu_se), width=0, size=0.3, show.legend = F) +
...
# Calculated Anova and Tukey test - Compare Par_ue, WL, Phase, Strain
...
MCGrowthRateStats$Phase <- factor(MCGrowthRateStats$Phase)

# Two way Anova with interactions
model<-aov(deltaOD_Lmu_corr~Par_ue*WL*Strain*Phase, data=MCGrowthRateStats)
AnovaTest_GrowthRate<-data.frame(unclass(summary(model)), check.names = FALSE, stringsAsFactors = FALSE)
TukeyHSDTest_GrowthRate<-TukeyHSD(model, which = c("Par_ue", "WL", "Strain", "Phase"))
```
...
```{r save rds}
# saveRDS(MultiCultiGrowth, file.path(DataOut, paste(Project, "Processed_MultiCultiGrowth.Rds", sep = "_"), fsep = .Platform$file.sep), ascii = FALSE, version = NULL, compress = "xz", refhook = NULL)
...
# Create a nice growth rate plot with the emission spectra
...
JazDataFile <- "../Data/ImportedData/ImportedJazEmData/BalticCCA_Imported_JazEmData.Rds"
...
```{r}
MultiCultiGrowthPlot<-MultiCultiGrowth %>% 
  mutate(WLNum = if_else(WL == "WW", 750, as.numeric(WL))) %>%
  filter(WL != "WW") 
```
...
```{r}
MultiCultiGrowthPlot <- MultiCultiGrowthPlot %>%
  filter(deltaOD_Lmu_corr > 0) %>% 
  filter(Run == 70 | Run == 0) 
```

```{r}
MultiCultiGrowthPlot$facetsPar_ue = factor(MultiCultiGrowthPlot$O2, labels = c("PAR~(µmol~photons~m^{-2}~s^{-1})"))
MultiCultiGrowthPlot$facetsPhotoperiod = factor(MultiCultiGrowthPlot$O2, labels = c("Photoperiod~(h)"))
MultiCultiGrowthPlot$facetsStrain = factor(MultiCultiGrowthPlot$O2, labels = c("Strain"))
...
JazDataRoundMeta %>%
  ...
  geom_point(aes(x = WLNum, y = deltaOD_Lmu_corr, colour = as.factor(Phase)), size = 4, show.legend = T, MultiCultiGrowthPlot) +
  geom_line(aes(x = WLNum, y = deltaOD_Lmu_corr, linetype = as.factor(Phase)), show.legend = T, MultiCultiGrowthPlot) +

  geom_errorbar(aes(x = WLNum, ymin = deltaOD_Lmu_corr - deltaOD_Lmu_se, ymax = deltaOD_Lmu_corr + deltaOD_Lmu_se), width=0, size=0.3, show.legend = F, MultiCultiGrowthPlot) +
  ...
  stat_wl_strip(aes(x = nm), ymin = -Inf, ymax = -0.030, alpha = 0.5) +
  scale_fill_identity() +
  labs(y = "µ " ~ "("~d^-1~") and LED emission spectra", x = "Growth waveband (nm)") +
...
# Super stupid way to make LED emmision at 60 and 300. Think Sylwia to better way to do this.

```{r}
JazDataRoundMeta60<-JazDataRoundMeta %>% 
    mutate(Par_ue=case_when(ObsDateJaz=="2022-11-30"~60,
         ObsDateJaz=="2022-11-30"~60))
JazDataRoundMeta60_647<-JazDataRoundMeta60 %>% 
  filter(Par_ue == 60) %>% 
  filter(WL == 660) %>% 
select(c(WL, nm, ObsDateJaz, EmNormJaz647, Par_ue))

JazDataRoundMeta60_521<-JazDataRoundMeta60 %>% 
  filter(Par_ue == 60) %>% 
  filter(WL == 530) %>% 
select(c(WL, nm, ObsDateJaz, EmNormJaz521, Par_ue))



JazDataRoundMeta300<-JazDataRoundMeta %>% 
    mutate(Par_ue=case_when(ObsDateJaz=="2022-11-30"~300,
         ObsDateJaz=="2022-11-30"~300))
JazDataRoundMeta300_647<-JazDataRoundMeta300 %>% 
  filter(Par_ue == 300) %>% 
  filter(WL == 660) %>% 
  select(c(WL, nm, ObsDateJaz, EmNormJaz647, Par_ue))

JazDataRoundMeta300_521<-JazDataRoundMeta300 %>% 
  filter(Par_ue == 300) %>% 
  filter(WL == 530) %>% 
  select(c(WL, nm, ObsDateJaz, EmNormJaz521, Par_ue))


JazDataRoundMeta60300<- rbind(JazDataRoundMeta60, JazDataRoundMeta300)


```

```{r}
JazDataRoundMeta60300$facetsPar_ue = factor(JazDataRoundMeta60300$ObsDateJaz, labels = c("PAR~(µmol~photons~m^{-2}~s^{-1})"))

JazDataRoundMeta60_647$facetsPar_ue = factor(JazDataRoundMeta60_647$ObsDateJaz, labels = c("PAR~(µmol~photons~m^{-2}~s^{-1})"))

JazDataRoundMeta60_521$facetsPar_ue = factor(JazDataRoundMeta60_521$ObsDateJaz, labels = c("PAR~(µmol~photons~m^{-2}~s^{-1})"))

JazDataRoundMeta300_647$facetsPar_ue = factor(JazDataRoundMeta300_647$ObsDateJaz, labels = c("PAR~(µmol~photons~m^{-2}~s^{-1})"))

JazDataRoundMeta300_521$facetsPar_ue = factor(JazDataRoundMeta300_521$ObsDateJaz, labels = c("PAR~(µmol~photons~m^{-2}~s^{-1})"))
...
JazDataRoundMeta60300 %>%
   ggplot() +
  #geom_area(aes(x = nm, y = EmNormJaz722, fill = "indianred4"), alpha = 0.1, show.legend = F) +
  geom_area(aes(x = nm, y = EmNormJaz647*0.6, fill = "brown1"), alpha = 0.1, show.legend = F, JazDataRoundMeta60_647) +
  #geom_area(aes(x = nm, y = EmNormJaz612, fill = "coral"), alpha = 0.1, show.legend = F) +
  geom_area(aes(x = nm, y = EmNormJaz521*0.6, fill = "limegreen"), alpha = 0.1, show.legend = F, JazDataRoundMeta60_521) +
  
    geom_area(aes(x = nm, y = EmNormJaz647*3, fill = "brown1"), alpha = 0.1, show.legend = F, JazDataRoundMeta300_647) +
  #geom_area(aes(x = nm, y = EmNormJaz612, fill = "coral"), alpha = 0.1, show.legend = F) +
  geom_area(aes(x = nm, y = EmNormJaz521*3, fill = "limegreen"), alpha = 0.1, show.legend = F, JazDataRoundMeta300_521) +
...
# Changing Run0 to Run 70. ExpDate need to be fix as well here!
```{r}
# MultiCultiGrowthSelect <-MultiCultiGrowth %>% 
#   filter(Run == 70 | Run == 0) 
# 
# MultiCultiGrowthSelect <-MultiCultiGrowthSelect %>% 
#     mutate(Run=case_when(Run==70~70,
#          Run==0~70))
# 
# MultiCultiGrowthSelect <-MultiCultiGrowthSelect %>% 
#     mutate(SampleID=case_when(SampleID=="SySl1242"~"SySl1242", SampleID=="SySl1243"~"SySl1243", SampleID=="SySl1244"~"SySl1244", SampleID=="SySl1245"~"SySl1245", SampleID=="SySl1246"~"SySl1246", SampleID=="SySl1247"~"SySl1247", SampleID=="SySl1248"~"SySl1248", SampleID=="SySl1249"~"SySl1249", 
#                               
#                                  SampleID=="SySl1348"~"SySl1242", SampleID=="SySl1349"~"SySl1243", SampleID=="SySl1350"~"SySl1244", SampleID=="SySl1351"~"SySl1245", SampleID=="SySl1352"~"SySl1246", SampleID=="SySl1353"~"SySl1247", SampleID=="SySl1354"~"SySl1248", SampleID=="SySl1355"~"SySl1249" 
#                          
#                               ))
```


# Create growth rate dataset to match PUR
```{r}
MultiCultiGrowthSelect <- MultiCultiGrowth %>%
  filter(deltaOD_Lmu_corr > 0) %>% 
  filter(Run == 70 | Run == 0) %>% 
  select(c(Strain, Par_ue, Photoperiod, WL, O2, PARPhotonDose_day, OD720_Lmu_se, OD720_Lmu_corr, deltaOD_Lmu_se, deltaOD_Lmu_corr, Phase))
```


# Create a prelim plot
```{r}

MultiCultiGrowthSelect %>%
  ggplot() +
  geom_point(aes(x = WL, y = deltaOD_Lmu_corr,colour=as.factor(Phase)), size = 3.5, alpha = 0.9, show.legend = T) +
  #geom_line(aes(x = WL, y = deltaOD_Lmu_corr, linetype=as.factor(Phase)), size = 1, alpha = 0.9, show.legend = T) +
  # geom_smooth(aes(x = WL, y = deltaOD_Lmu_corr, linetype=as.factor(Phase)), size = 1, alpha = 0.2, show.legend = T, method = "lm") +
...
```{r read imported pigments Exp file}
OlisFile <- "../Data/ProcessedData/ProcessedOlisJazData/BalticCCA_Processed_OlisSpectraTidy.Rds"
...
# Create a dataset to match growth rate - only run 68 and 70, day 2
```{r}
# OlisSelect <- Olis %>%
#   filter(Phase!= "St") %>% 
#   filter(WL!= "WW") %>% 
...
# Create a dataset to match all runs and all PUR data in each phase. 
...
# Combine growth rate and PUR
# Only exp and resupply phase because growth rate doeasn't have stationary phase!
...
MCGrowthRateSelect <- MultiCultiGrowthSelect %>% 
  left_join(., OlisSelect, by = c("Strain" = "Strain", "Par_ue" = "Par_ue", "Photoperiod" = "Photoperiod",  "O2" = "O2", "WL" = "WL", "PARPhotonDose_day" = "PARPhotonDose_day", "Phase" = "Phase")) 
  

```

```{r}
MCGrowthRateSelect$facetsPar_ue = factor(MCGrowthRateSelect$O2, labels = c("PAR~(µmol~photons~m^{-2}~s^{-1})"))
MCGrowthRateSelect$facetsPhotoperiod = factor(MCGrowthRateSelect$O2, labels = c("Photoperiod~(h)"))
MCGrowthRateSelect$facetsStrain = factor(MCGrowthRateSelect$O2, labels = c("Strain"))
```


#Prelim plot u vs PUR
```{r}
MCGrowthRateSelect %>%
  ggplot() +
  geom_point(aes(x = PUR, y = deltaOD_Lmu_corr,colour=as.factor(Phase)), size = 3.5, alpha = 0.9, show.legend = T) +
    # geom_smooth(aes(x = PUR, y = deltaOD_Lmu_corr, linetype=as.factor(Phase)), size = 1, alpha = 0.2, show.legend = T, method = "lm") +
  #geom_line(aes(x = WL, y = deltaOD_Lmu_corr, linetype=as.factor(Strain)), size = 1, alpha = 0.9, show.legend = T) +
  geom_errorbar(aes(x = PUR, ymin = deltaOD_Lmu_corr - deltaOD_Lmu_se, ymax = deltaOD_Lmu_corr + deltaOD_Lmu_se), width=0, size=0.3, show.legend = F) +
...
```{r}
MCGrowthRateSelect<-MCGrowthRateSelect %>%     
...
MCGrowthRateSelect %>%
  ...
  ggplot() +
  geom_point(aes(x = PUR, y = deltaOD_Lmu_corr,colour=as.factor(Phase)), size = 3.5, alpha = 0.9, show.legend = T) +
  geom_smooth(aes(x = PUR, y = deltaOD_Lmu_corr, linetype=as.factor(Phase)), colour = "black", size = 0.4, alpha = 0.2, show.legend = F, method = "lm") +
  #geom_line(aes(x = WL, y = deltaOD_Lmu_corr, linetype=as.factor(Strain)), size = 1, alpha = 0.9, show.legend = T) +
  geom_errorbar(aes(x = PUR, ymin = deltaOD_Lmu_corr - deltaOD_Lmu_se, ymax = deltaOD_Lmu_corr + deltaOD_Lmu_se), width=0, size=0.3, show.legend = F) +
    scale_colour_manual(values = c("slateblue4", "skyblue3")) +
...
MCGrowthRateSelect %>%
  ...
  geom_errorbar(aes(x = PUR, ymin = deltaOD_Lmu_corr - deltaOD_Lmu_se, ymax = deltaOD_Lmu_corr + deltaOD_Lmu_se), width=0, size=0.3, show.legend = F) +
    scale_colour_manual(values = c("slateblue4", "skyblue3")) +
    labs(y = "µ " ~ "("~d^-1~")", x = "PUR ( µmol photons "~m^-2~""~s^-1~")") +
  ggh4x::facet_nested(rows = vars(facetsStrain, Strain), cols = vars(WLnm), labeller = label_parsed) +
  theme_bw() +
    theme(panel.grid.minor = element_blank(),
        panel.grid.major = element_blank(),
        #panel.spacing.x = unit(0.4, 'cm'),
...
MCGrowthRateSelect %>%
  ...
  geom_errorbar(aes(x = PUR, ymin = deltaOD_Lmu_corr - deltaOD_Lmu_se, ymax = deltaOD_Lmu_corr + deltaOD_Lmu_se), width=0, size=0.3, show.legend = F) +
    scale_colour_manual(values = c("slateblue4", "skyblue3")) +
    labs(y = "µ " ~ "("~d^-1~")", x = "PUR ( µmol photons "~m^-2~""~s^-1~")") +
  ggh4x::facet_nested(rows = vars(facetsStrain, Strain), cols = vars(Par_ue, WLnm), labeller = label_parsed) +
  theme_bw() +
    theme(panel.grid.minor = element_blank(),
        panel.grid.major = element_blank(),
        #panel.spacing.x = unit(0.4, 'cm'),
...
```{r}
MCGrowthRateSelectlinear <- MCGrowthRateSelect %>%   
  #drop_na(Par_ue, Ex_WL, Phase, meanSig_nm2psii, AllmeanPhycoChlaRatio) %>% 
  select(c(Strain, deltaOD_Lmu_corr, PUR, Phase, Par_ue, WL)) %>%  
  nest(data = -c("Phase", "Strain")) %>%   
  mutate(Linear_Model = map(data, ~lm(deltaOD_Lmu_corr ~ 0 + PUR, data = .x))) %>%     
...
# Statistic has been made for comparison all data combined (without separation to WL)!!!!!


t-test from the linear model Phase
```{r}
# Strain across Phase
Stats77_ExpSt<-MCGrowthRateSelect %>%
  filter(Strain == "PC-rich") 
  fit <- lm(deltaOD_Lmu_corr ~ PUR + Phase + PUR:Phase, data=Stats77_ExpSt)
...
model_77021<-data.frame(unclass(x), check.names = FALSE, stringsAsFactors = FALSE) %>% 
  ...
  model_77021$FitComparison <- 'PC-rich_Exp:ExpResupply'


Stats127_ExpSt<-MCGrowthRateSelect %>%
  filter(Strain == "PE-rich") 
  fit <- lm(deltaOD_Lmu_corr ~ PUR + Phase + PUR:Phase, data=Stats127_ExpSt)
x <- cbind(t(as.numeric(coefficients(fit))), t(as.numeric(summary(fit)$coefficients[4, 4])), t(as.numeric(summary(fit)$coefficients[4, 3])), t(as.numeric(summary(fit)$coefficients[4, 2])))
model_127021<-data.frame(unclass(x), check.names = FALSE, stringsAsFactors = FALSE) %>% 
  rename("Estimate" = "4", "Std.Error" = "7", "t_value" = "6", "p_value" = "5") %>% 
  select(c("Estimate", "Std.Error", "t_value", "p_value"))
  model_127021$FitComparison <- 'PE-rich_Exp:ExpResupply'

modelPhase<-rbind(model_77021, model_127021)
...
Stats77_ExpSt<-MCGrowthRateSelect %>%
  filter(Phase == "Exp") 
  fit <- lm(deltaOD_Lmu_corr ~ PUR + Strain + PUR:Strain, data=Stats77_ExpSt)
x <- cbind(t(as.numeric(coefficients(fit))), t(as.numeric(summary(fit)$coefficients[4, 4])), t(as.numeric(summary(fit)$coefficients[4, 3])), t(as.numeric(summary(fit)$coefficients[4, 2])))
model_077127<-data.frame(unclass(x), check.names = FALSE, stringsAsFactors = FALSE) %>% 
  rename("Estimate" = "4", "Std.Error" = "7", "t_value" = "6", "p_value" = "5") %>% 
  select(c("Estimate", "Std.Error", "t_value", "p_value"))
  model_077127$FitComparison <- 'Exp_PC-rich:PE-rich'


Stats127_ExpSt<-MCGrowthRateSelect %>%
  filter(Phase == "ExpResupply") 
  fit <- lm(deltaOD_Lmu_corr ~ PUR + Strain + PUR:Strain, data=Stats127_ExpSt)
x <- cbind(t(as.numeric(coefficients(fit))), t(as.numeric(summary(fit)$coefficients[4, 4])), t(as.numeric(summary(fit)$coefficients[4, 3])), t(as.numeric(summary(fit)$coefficients[4, 2])))
model_2177127<-data.frame(unclass(x), check.names = FALSE, stringsAsFactors = FALSE) %>% 
  rename("Estimate" = "4", "Std.Error" = "7", "t_value" = "6", "p_value" = "5") %>% 
  select(c("Estimate", "Std.Error", "t_value", "p_value"))
  model_2177127$FitComparison <- 'ExpResupply_PC-rich:PE-rich'
...
# Merge statistic
```{r}
Anova_GrowthPUR<-rbind(modelPhase, modelStrain)
...
```{r}
saveRDS(Anova_GrowthPUR, file.path(TableRdsPath, paste(Project, "Anova_GrowthPUR_Model.Rds", sep = "_"), fsep = .Platform$file.sep), ascii = FALSE, version = NULL, compress = "xz", refhook = NULL)
```
...
```{r}
GrowthPURPEExp<-MCGrowthRateSelect %>% 
  filter(Strain == "PE-rich") %>% 
  filter(Phase == "Exp")
PEExplm<-lm(deltaOD_Lmu_corr~ 0 + PUR, data=GrowthPURPEExp)
GrowthPURPEExplm<-summary(PEExplm)
# Coefficients:
#        Estimate   Std. Error  t value     Pr(>|t|)
# PUR 0.007894869 0.0002693082 29.31537 8.717029e-05


GrowthPURPEExpResupply<-MCGrowthRateSelect %>% 
  filter(Strain == "PE-rich") %>% 
  filter(Phase == "ExpResupply")
PEExpResupplylm<-lm(deltaOD_Lmu_corr~ 0 + PUR, data=GrowthPURPEExpResupply)
GrowthPURPEExpResupplylm<-summary(PEExpResupplylm)
# Coefficients:
#        Estimate   Std. Error  t value    Pr(>|t|)
# PUR 0.005986438 0.0006696453 8.939715 0.002953076


GrowthPURPCExp<-MCGrowthRateSelect %>% 
  filter(Strain == "PC-rich") %>% 
  filter(Phase == "Exp")
PCExplm<-lm(deltaOD_Lmu_corr~ 0 + PUR, data=GrowthPURPCExp)
GrowthPURPCExplm<-summary(PCExplm)
# Coefficients:
#       Estimate  Std. Error  t value   Pr(>|t|)
# PUR 0.01173578 0.002323907 5.050023 0.01497774


GrowthPURPCExpResupply<-MCGrowthRateSelect %>% 
  filter(Strain == "PC-rich") %>% 
  filter(Phase == "ExpResupply")
PCExpResupplylm<-lm(deltaOD_Lmu_corr~ 0 + PUR, data=GrowthPURPCExpResupply)
GrowthPURPCExpResupplylm<-summary(PCExpResupplylm)
# Coefficients:
#        Estimate  Std. Error  t value  Pr(>|t|)
# PUR 0.006886862 0.001931022 3.566433 0.0376474
...
```{r save rds}
saveRDS(MCGrowthRateSelect, file.path(DataOut, paste(Project, "Processed_GrowthRatePURSelect.Rds", sep = "_"), fsep = .Platform$file.sep), ascii = FALSE, version = NULL, compress = "xz", refhook = NULL)
...
# Plot Delta with fancy looking legend
