...
JazDataRoundMeta %>%
  ...
  scale_fill_identity() +
  labs(y = "µ " ~ "("~d^-1~") and normalized LED emission spectra", x = "Growth waveband (nm)") +
...
MCGrowthRateSelect %>%
  ...
  ggplot() +
  geom_point(aes(x = PUR, y = deltaOD_Lmu_corr,colour=as.factor(Phase)), size = 3.5, alpha = 0.9, show.legend = T) +
  geom_smooth(aes(x = PUR, y = deltaOD_Lmu_corr, linetype=as.factor(Phase)), colour = "black", size = 0.4, alpha = 0.2, show.legend = T, method = "lm") +
  #geom_line(aes(x = WL, y = deltaOD_Lmu_corr, linetype=as.factor(Strain)), size = 1, alpha = 0.9, show.legend = T) +
  geom_errorbar(aes(x = PUR, ymin = deltaOD_Lmu_corr - deltaOD_Lmu_se, ymax = deltaOD_Lmu_corr + deltaOD_Lmu_se), width=0, size=0.3, show.legend = F) +
    scale_colour_manual(values = c("slateblue4", "skyblue3")) +
    labs(y = "µ " ~ "("~d^-1~")", x = "PUR ( µmol photons "~m^-2~""~s^-1~")") +
  ggh4x::facet_nested(rows = vars(facetsStrain, Strain), labeller = labeller(Ex_WL = label_value, WL = label_value)) +
  theme_bw() +
    theme(panel.grid.minor = element_blank(),
        panel.grid.major = element_blank(),
        #panel.spacing.x = unit(0.4, 'cm'),
...
MCGrowthRateSelect %>%
  ...
  geom_errorbar(aes(x = PUR, ymin = deltaOD_Lmu_corr - deltaOD_Lmu_se, ymax = deltaOD_Lmu_corr + deltaOD_Lmu_se), width=0, size=0.3, show.legend = F) +
    scale_colour_manual(values = c("slateblue4", "skyblue3")) +
    labs(y = "µ " ~ "("~d^-1~")", x = "PUR ( µmol photons "~m^-2~""~s^-1~")") +
  ggh4x::facet_nested(rows = vars(facetsStrain, Strain), cols = vars(WL), labeller = labeller(Ex_WL = label_value, WL = label_value)) +
  theme_bw() +
    theme(panel.grid.minor = element_blank(),
        panel.grid.major = element_blank(),
        #panel.spacing.x = unit(0.4, 'cm'),
